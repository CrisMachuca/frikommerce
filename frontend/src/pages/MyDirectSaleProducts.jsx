...
const MyDirectSaleProducts = () => {
    ...
    return (
        <div>
            <h2 className="text-2xl font-bold mb-4">Mis Productos de Venta Directa</h2>
            <ul className="space-y-6">
                {products.map((product) => (
                    <li key={product.id} className="bg-white p-4 shadow rounded">
                        ...
                        {product.image_url && (
                            <img src={product.image_url} alt={product.name} className="w-full h-auto mt-4 rounded-lg" />
                        )}
